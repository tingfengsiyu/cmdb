...
func (sws *LogicSshWsSession) receiveWsMsg(exitCh chan bool) {
	...
	for {
		...
		default:
			...
			if err != nil {
				logrus.SugarLogger.Error(err, "reading webSocket message failed")
				return
			}
			//unmashal bytes into struct
			msgObj := wsMsg{}
			if err := json.Unmarshal(wsData, &msgObj); err != nil {
				logrus.SugarLogger.Error(err, "wsData", string(wsData), "unmarshal websocket message failed")
			}
			switch msgObj.Type {
			case wsMsgResize:
				...
				if msgObj.Cols > 0 && msgObj.Rows > 0 {
					if err := sws.session.WindowChange(msgObj.Rows, msgObj.Cols); err != nil {
						logrus.SugarLogger.Error(err, "ssh pty change windows size failed")
					}
				}
			case wsMsgCmd:
				...
				if err != nil {
					logrus.SugarLogger.Error(err, "websock cmd string base64 decoding failed")
				}
...
func (sws *LogicSshWsSession) sendWebsocketInputCommandToSshSessionStdinPipe(cmdBytes []byte) {
	if _, err := sws.stdinPipe.Write(cmdBytes); err != nil {
		logrus.SugarLogger.Error(err, "ws cmd bytes write to ssh.stdin pipe failed")
	}
}
	...
	for {
		...
		case <-tick.C:
			...
			if len(bs) > 0 {
				...
				if err != nil {
					logrus.SugarLogger.Error(err, "ssh sending combo output to webSocket failed")
				}
				_, err = sws.logBuff.Write(bs)
				if err != nil {
					logrus.SugarLogger.Error(err, "combo output to log buffer failed")
				}
...
func (sws *LogicSshWsSession) Wait(quitChan chan bool) {
	if err := sws.session.Wait(); err != nil {
		logrus.SugarLogger.Error(err, "ssh session wait failed")
		setQuit(quitChan)
	}
}
